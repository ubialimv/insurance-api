...
import DisabilityEngine from '../disability.engine';
...
const engine = new DisabilityEngine(commonRulesEngine);
...
describe('disability engine', () => {
  it('should execute hasIncome in order to check Eligibility', () => {
    const spy = jest.spyOn(user, 'hasIncome');
  ...
  describe('when user is eligible', () => {
    ...
    it('should calculate score based on hasDependents returning true', () => {
      ...
      jest.spyOn(user, 'hasHouseAndIsMortgaged').mockReturnValue(false);
      jest.spyOn(user, 'isMarried').mockReturnValue(false);
